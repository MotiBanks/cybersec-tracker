...
export const TasksService = {
  ...
  async createTask(task: InsertTask): Promise<Task> {
    ...
    try {
      // First, get the current user to ensure we have the right permissions
      const { data: userData } = await supabase.auth.getUser()
      if (!userData?.user) {
        throw new Error("User not authenticated")
      }
      
      // IMPORTANT: Always use the current authenticated user's ID
      // This is critical for Row Level Security policies
      task.user_id = userData.user.id
      
      // CRITICAL: Ensure user profile exists before creating task
      ...
      if (!userProfile) {
        ...
        await userService.getUserProfile(task.user_id)
        ...
        if (checkError || !checkProfile) {
          console.error("Failed to create user profile before task creation")
