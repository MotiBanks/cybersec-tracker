...
export async function GET() {
  try {
    ...
    if (badgesError) {
      ...
      return NextResponse.json({ success: false, error: "Failed to seed badges" }, { status: 500 })
    }
    ...
    if (languagesError) {
      ...
      return NextResponse.json({ success: false, error: "Failed to seed languages" }, { status: 500 })
    }
    ...
    const { error: moodSuggestionsError } = await supabase
      ...
      .upsert(moodSuggestions, { onConflict: ["mood_type", "suggestion"] })

    if (moodSuggestionsError) {
      console.error("Error seeding mood suggestions:", moodSuggestionsError)
      return NextResponse.json({ success: false, error: "Failed to seed mood suggestions" }, { status: 500 })
    }
    ...
    if (hackChallengesError) {
      ...
      return NextResponse.json({ success: false, error: "Failed to seed hack challenges" }, { status: 500 })
    }
